...
class TTSModel:
    ...
    async def tts_synthesis_chunked(self, data : str)-> AsyncGenerator:
        ...
        for line in lines:
            ...
            if line != '':
                ...
                sf.write(io_buffer, speech.numpy(), samplerate=self.config["audio"]["sample_rate"],subtype="PCM_16",format = "RAW")
                end = io_buffer.tell()
    ...
    async def tts_synthesis(self, data : str)-> AsyncGenerator:
        ...
        speech = self.model.generate_speech(inputs["input_ids"], self.speaker_embeddings, vocoder=self.vocoder)
        sf.write(io_buffer, speech.numpy(), samplerate=self.config["audio"]["sample_rate"],subtype="PCM_16",format = "WAV")
